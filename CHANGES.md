...
Fix function `count_comps()`. To explain the fixes, we assume that the path is
...
`foo` then it is incorrect to say that there are two directories. This could be
deemed problematic in some cases and it might be better to have a boolean to
decide but for now that is not the case.
...
were added to test the above change with how components are counted.
