 ...
 *
 * FIXME: if level < 0 it is supposed to return the first component but some
 * problematic edge cases were discovered and need to be looked at.
 */
char *
dir_name(char const *path, int level)
{
	...
	return copy;
        ...
        /*
         * FIXME: in some conditions this is broken.
         */
        p = strchr(copy, '/');
        if (p != NULL) {
            *p = '\0';
...
type_of_file(char const *path)
{
     ...
     */
    err(111, __func__, "path is unknown file type: %s", path);
    not_reached();
}
...
fts_path(FTSENT *ent)
{
    ...
    if (ent == NULL) {
         ...
         * Treebeard died :(
         */
        err(124, __func__, "ent is NULL");
        not_reached();
    }
    if (ent->fts_path == NULL) {
        err(125, __func__, "ent->fts_path == NULL");
        not_reached();
    ...
    switch (ent->fts_info) {
        case FTS_DC: /* cycle in directory tree */
            ...
            if (fts_set(fts, ent, FTS_SKIP) != 0) {
                errp(133, __func__, "failed to set FTS_SKIP on a directory that causes a cycle in the tree: %s", ent->fts_path);
                not_reached();
            } else {
        ...
        case FTS_DNR: /* directory not readable */
            errno = 0; /* pre-clear errno for errp() */
            if (fts_set(fts, ent, FTS_SKIP) != 0) {
                errp(134, __func__, "failed to set FTS_SKIP on an unreadable directory in the tree: %s", ent->fts_path);
                not_reached();
            } else {
        ...
        case FTS_NS: /* no stat(2) info available but we requested it */
            err(135, __func__, "no stat(2) info available for %s in tree", ent->fts_path);
            not_reached();
            break;
        case FTS_NSOK: /* stat(2) not requested */
            err(136, __func__, "stat(2) info not requested for %s in tree: FTS_NOSTAT set!", ent->fts_path);
            not_reached();
            break;
        case FTS_ERR: /* some error condition */
            ...
            errp(137, __func__, "encountered error from path %s in tree", ent->fts_path);
            not_reached();
            break;
        default:
        ...
        int (*cmp)(const FTSENT **, const FTSENT **), bool(*check)(FTS *, FTSENT *))
{
    ...
    if (dir == NULL && dirfd < 0 && cwd != NULL && options < 0 && fts == NULL && cmp == NULL &&
        ...
        if (*cwd > 0) {
            errno = 0;  /* pre-clear errno for errp() */
            if (fchdir(*cwd) != 0) {
                errp(138, __func__, "failed to fchdir(%d)", *cwd);
                not_reached();
            }
            errno = 0; /* pre-clear errno for errp() */
            if (close(*cwd) != 0) {
                errp(139, __func__, "close(%d) failed", *cwd);
                not_reached();
            }
            *cwd = -1;
            return NULL;
    ...
    if (dir != NULL && *dir != '\0') {
        ...
        if (chdir(dir) != 0) {
            ...
            if (dirfd > 0) {
                errno = 0;  /* pre-clear errno for errp() */
                if (fchdir(dirfd) != 0) {
                    errp(142, __func__, "both chdir(\"%s\") and fchdir(%d) failed",
                            dir, dirfd);
                    not_reached();
                }
            }
        ...
        errno = 0; /* pre-clear errno for errp() */
        if (fchdir(dirfd) != 0) {
            errp(143, __func__, "fchdir(%d) failed", dirfd);
            not_reached();
        }
    }
    ...
    if (*fts == NULL) {
        *fts = fts_open(path, options, cmp != NULL ? cmp : fts_cmp);
        if (*fts == NULL) {
            errp(144, __func__, "fts_open() returned NULL for: %s", dir != NULL ? dir : ".");
            not_reached();
        }
    }
    /*
    ...
    if (*fts != NULL) {
                ...
                *fts = NULL;
                ...
                switch (ent->fts_info) {
                    case FTS_DC: /* cycle in directory tree */
                        errno = 0; /* pre-clear errno for errp() */
                        if (fts_set(*fts, ent, FTS_SKIP) != 0) {
                            errp(145, __func__, "failed to set FTS_SKIP on a directory that causes a cycle in the tree: %s",
                                    ent->fts_path);
                            not_reached();
                        } else {
                            warn(__func__, "skipping directory %s because it causes a cycle in the tree", ent->fts_path);
                    ...
                    case FTS_DNR: /* directory not readable */
                        errno = 0; /* pre-clear errno for errp() */
                        if (fts_set(*fts, ent, FTS_SKIP) != 0) {
                            errp(146, __func__, "failed to set FTS_SKIP on an unreadable directory the tree: %s", ent->fts_path);
                            not_reached();
                        } else {
                            warn(__func__, "skipping unreadable directory %s in the tree", ent->fts_path);
                            continue;
                        ...
                        break;
                    case FTS_NS: /* no stat(2) info available but we requested it */
                        err(147, __func__, "no stat(2) info available for %s in tree", ent->fts_path);
                        not_reached();
                        break;
                    case FTS_NSOK: /* stat(2) was not requested */
                        err(148, __func__, "stat(2) not requested: FTS_NOSTAT set");
                        not_reached();
                        break;
                    case FTS_ERR: /* some error condition */
                        /*
                         ...
                         */
                        errno = ent->fts_errno; /* pre-clear errno for errp() */
                        errp(149, __func__, "encountered error from path %s in tree", ent->fts_path);
                        not_reached();
                        break;
                    default:
                        /* if the caller needs to check other conditions they can do
        ...
        err(150, __func__, "*fts is NULL when it shouldn't be");
        not_reached();
    }
    return NULL;
}
        ...
        int (*cmp)(const FTSENT **, const FTSENT **), bool(*check)(FTS *, FTSENT *))
{
    ...
    if (ent == NULL){
        err(151, __func__, "failed to open \".\"");
        not_reached();
    } else {
        i = 0;
        do {
            ...
            if (*path == '\0') {
                if (count <= 0 || (count > 0 && ++i == count)) {
                    ...
                    if (dir == NULL && cwd != NULL && *cwd < 0) {
                        ...
                        if (d == NULL) {
                            errp(152, __func__, "failed to get current working directory");
                            not_reached();
                        }
                        path_found = calloc_path(d, p);
                        if (path_found == NULL) {
                            err(153, __func__, "failed to allocate absolute path of %s", p);
                            not_reached();
                        }
                        if (d != NULL) {
                        ...
                        path_found = strdup(p);
                        if (path_found == NULL) {
                            errp(154, __func__, "failed to strdup(\"%s\")", p);
                            not_reached();
                        }
                    }
                    /*
                ...
                if (base && !strcmp(ent->fts_name, path)) {
                    if (count <= 0 || (count > 0 && ++i == count)) {
                        ...
                        path_found = strdup(p);
                        if (path_found == NULL) {
                            errp(155, __func__, "failed to strdup(\"%s\")", p);
                            not_reached();
                        }
                        /*
                         * close down stream
...
append_path(struct dyn_array **paths, char *str, bool unique, bool duped)
{
     ...
     */
    if (paths == NULL) {
	err(157, __func__, "paths is NULL");
	not_reached();
    }
    if (str == NULL) {
	err(158, __func__, "str is NULL");
	not_reached();
    }
    ...
    if (*paths == NULL) {
        ...
        *paths  = dyn_array_create(sizeof(char *), 64, 64, true);
        if (*paths == NULL) {
            err(159, __func__, "failed to create paths paths");
            not_reached();
        }
    }
    ...
    if (unique) {
        ...
        for (i=0; i < unique_len; ++i) {
            ...
            if (u == NULL) {	/* paranoia */
                err(160, __func__, "found NULL pointer in paths dynamic array element: %ju", (uintmax_t)i);
                not_reached();
            }
    ...
    if (!duped) {
        ...
        u = strdup(str);
        if (u == NULL) {
            errp(161, __func__, "failed to strdup(\"%s\")", str);
            not_reached();
        }
    } else {
        u = str;
        ...
        int (*cmp)(const FTSENT **, const FTSENT **), bool(*check)(FTS *, FTSENT *))
{
     ...
     */
    if (paths == NULL) {
        err(162, __func__, "paths list is NULL");
        not_reached();
    }
    ...
    if (dir == NULL && dirfd < 0) {
        ...
        dir = getcwd(NULL, 0);
        if (dir == NULL) {
            errp(163, __func__, "failed to get absolute path");
            not_reached();
        }
    ...
    ent = read_fts(dir, dirfd, cwd, options, logical, &fts, cmp, check);
    if (ent == NULL){
        err(164, __func__, "failed to open \".\"");
        not_reached();
    } else {
        i = 0;
        do {
            ...
            for (j = 0; j < len; ++j) {
                ...
                if (path == NULL) {
                    err(165, __func__, "paths[%ju] == NULL", j);
                    not_reached();
                }
                if (*path == '\0') {
                    if (count <= 0 || (count > 0 && ++i == count)) {
                        ...
                        if (abspath) {
                            ...
                            if (name == NULL) {
                                err(166, __func__, "failed to allocate path: %s/%s", dir, p);
                                not_reached();
                            } else {
                    ...
                    if (count <= 0 || (count > 0 && ++i == count)) {
                        ...
                        if (abspath) {
                            name = calloc_path(dir, p);
                            if (name == NULL) {
                                err(167, __func__, "failed to allocate path: %s/%s", dir, p);
                                not_reached();
                            } else {
                                dbg(DBG_VVHIGH, "allocated absolute path: %s", name);
                            }
...
filemode(char const *path)
{
     ...
     */
    if (path == NULL) {
	err(169, __func__, "called with NULL path");
	not_reached();
    }
...
flush_tty(char const *name, bool flush_stdin, bool abort_on_error)
{
	...
	if (fd_is_ready(name, true, fileno(stdin))) {
		...
		if (abort_on_error) {
		    errp(170, name, "fflush(stdin): error code: %d", ret);
		    not_reached();
		} else {
	...
	if (ret < 0) {
	    /* exit or error return depending on abort_on_error */
	    if (abort_on_error) {
		errp(171, name, "fflush(stdout): error code: %d", ret);
		not_reached();
	    } else {
	...
	if (ret < 0) {
	    /* exit or error return depending on abort_on_error */
	    if (abort_on_error) {
		errp(172, name, "fflush(stderr): error code: %d", ret);
		not_reached();
	    } else {
...
file_size(char const *path)
{
     ...
     */
    if (path == NULL) {
	err(173, __func__, "called with NULL path");
	not_reached();
    }
...
shell_cmd(char const *name, bool flush_stdin, bool abort_on_error, char const *format, ...)
{
	...
	/* exit or error return depending on abort_on_error */
	if (abort_on_error) {
	    err(174, __func__, "function name is not caller name because we were called with NULL name");
	    not_reached();
	} else {
	...
	/* exit or error return depending on abort_on_error */
	if (abort_on_error) {
	    err(175, name, "called with NULL format");
	    not_reached();
	} else {
	...
	/* exit or error return depending on abort_on_error */
	if (abort_on_error) {
	    errp(176, name, "calloc failed in vcmdprintf()");
	    not_reached();
	} else {
	...
	/* exit or error return depending on abort_on_error */
	if (abort_on_error) {
	    errp(177, __func__, "error calling system(\"%s\")", cmd);
	    not_reached();
	} else {
	...
	/* exit or error return depending on abort_on_error */
	if (abort_on_error) {
	    errp(178, __func__, "execution of the shell failed for system(\"%s\")", cmd);
	    not_reached();
	} else {
...
pipe_open(char const *name, bool write_mode, bool abort_on_error, char const *format, ...)
{
	...
	if (abort_on_error) {
	    err(179, __func__, "function name is not caller name because we were called with NULL name");
	    not_reached();
	} else {
	...
	/* exit or error return depending on abort */
	if (abort_on_error) {
	    err(180, name, "called with NULL format");
	    not_reached();
	} else {
	...
	/* exit or error return depending on abort */
	if (abort_on_error) {
	    errp(181, name, "calloc failed in vcmdprintf()");
	    not_reached();
	} else {
	    dbg(DBG_MED, "called from %s: calloc failed in vcmdprintf(): %s returning: %d < 0",
			 name, strerror(errno), EXIT_CALLOC_FAILED);
	...
	/* exit or error return depending on abort_on_error */
	if (abort_on_error) {
	    errp(182, name, "error calling popen(\"%s\", \"%s\")", cmd, write_mode?"w":"r");
	    not_reached();
	} else {
...
para(char const *line, ...)
{
     ...
     */
    if (stdout == NULL) {
	err(183, __func__, "stdout is NULL");
	not_reached();
    }
    clearerr(stdout);		/* pre-clear ferror() status */
    errno = 0;			/* pre-clear errno for errp() */
    ...
    fd = fileno(stdout);
    if (fd < 0) {
	errp(184, __func__, "fileno on stdout returned: %d < 0", fd);
	not_reached();
    }
	...
	if (ret == EOF) {
	    if (ferror(stdout)) {
		errp(185, __func__, "error writing paragraph to a stdout");
		not_reached();
	    } else if (feof(stdout)) {
		err(186, __func__, "EOF while writing paragraph to a stdout");
		not_reached();
	    } else {
		errp(187, __func__, "unexpected fputs error writing paragraph to stdout");
		not_reached();
	    }
	}
	...
	if (ret == EOF) {
	    if (ferror(stdout)) {
		errp(188, __func__, "error writing newline to stdout");
		not_reached();
	    } else if (feof(stdout)) {
		err(189, __func__, "EOF while writing newline to stdout");
		not_reached();
	    } else {
		errp(190, __func__, "unexpected fputc error writing newline to stdout");
		not_reached();
	    }
	}
    ...
    if (ret == EOF) {
	if (ferror(stdout)) {
	    errp(191, __func__, "error flushing stdout");
	    not_reached();
	} else if (feof(stdout)) {
	    err(192, __func__, "EOF while flushing stdout");
	    not_reached();
	} else {
	    errp(193, __func__, "unexpected fflush error while flushing stdout");
	    not_reached();
	}
    }
...
fpara(FILE * stream, char const *line, ...)
{
     ...
     */
    if (stream == NULL) {
	err(194, __func__, "stream is NULL");
	not_reached();
    }
    ...
    fd = fileno(stream);
    if (fd < 0) {
	errp(195, __func__, "fileno on stream returned: %d < 0", fd);
	not_reached();
    }
	...
	if (ret == EOF) {
	    if (ferror(stream)) {
		errp(196, __func__, "error writing paragraph to stream");
		not_reached();
	    } else if (feof(stream)) {
		err(197, __func__, "EOF while writing paragraph to stream");
		not_reached();
	    } else {
		errp(198, __func__, "unexpected fputs error writing paragraph to stream");
		not_reached();
	    }
	}
	...
	if (ret == EOF) {
	    if (ferror(stream)) {
		errp(199, __func__, "error writing newline to stream");
		not_reached();
	    } else if (feof(stream)) {
		err(200, __func__, "EOF while writing newline to stream");
		not_reached();
	    } else {
		errp(201, __func__, "unexpected fputc error writing newline to stream");
		not_reached();
	    }
	}
	++line_count;		/* count this line as printed */
    ...
    if (ret == EOF) {
	if (ferror(stream)) {
	    errp(202, __func__, "error flushing stream");
	    not_reached();
	} else if (feof(stream)) {
	    err(203, __func__, "EOF while flushing stream");
	    not_reached();
	} else {
	    errp(204, __func__, "unexpected fflush error while flushing stream");
	    not_reached();
	}
    }
...
readline(char **linep, FILE * stream)
{
	...
	if (feof(stream)) {
	    ...
	    errp(206, __func__, "getline() error");
	    not_reached();
	} else {
	    errp(207, __func__, "unexpected getline() error");
	    not_reached();
	}
    }
    /*
     ...
     */
    if (*linep == NULL) {
	err(208, __func__, "*linep is NULL after getline()");
	not_reached();
    }
...
readline_dup(char **linep, bool strip, size_t *lenp, FILE *stream)
{
    ...
    if (ret == NULL) {
	errp(210, __func__, "calloc of read line of %jd bytes failed", (intmax_t)len+1+1);
	not_reached();
    }
...
read_all(FILE *stream, size_t *psize)
{
     ...
     */
    if (stream == NULL) {
	err(211, __func__, "called with NULL stream");
	not_reached();
    }
...
copyfile(char const *src, char const *dest, bool copy_mode, mode_t mode)
{
     ...
     */
    if (src == NULL) {
        err(212, __func__, "src path is NULL");
        not_reached();
    } else if (*src == '\0') {
        err(213, __func__, "src path is empty string");
        not_reached();
    }

    if (dest == NULL) {
        err(214, __func__, "dest path is NULL");
        not_reached();
    } else if (*dest == '\0') {
        err(215, __func__, "dest path is empty string");
        not_reached();
    }
     ...
     */
    if (!exists(src)) {
        err(216, __func__, "src file does not exist: %s", src);
        not_reached();
    } else if (!is_file(src)) {
        err(217, __func__, "src file is not a regular file: %s", src);
        not_reached();
    } else if (!is_read(src)) {
        err(218, __func__, "src file is not readable: %s", src);
        not_reached();
    }
     ...
     */
    if (exists(dest)) {
        err(219, __func__, "dest file already exists: %s", dest);
        not_reached();
    }
    ...
    if (in_file == NULL) {
        errp(220, __func__, "couldn't open src file %s for reading: %s", src, strerror(errno));
        not_reached();
    }
    ...
    if (infd < 0) {
        errp(221, __func__, "failed to obtain file descriptor for %s: %s", src, strerror(errno));
        not_reached();
    }
    ...
    if (ret < 0) {
	errp(222, __func__, "failed to get stat info for %s, stat returned: %s", src, strerror(errno));
        not_reached();
    }
    ...
    if (buf == NULL) {
        err(223, __func__, "couldn't read in src file: %s", src);
        not_reached();
    }
    ...
    ret = fclose(in_file);
    if (ret < 0) {
	errp(224, __func__, "fclose error for %s: %s", src, strerror(errno));
	not_reached();
    }
    ...
    if (out_file == NULL) {
        ...
        errp(225, __func__, "couldn't open dest file %s for writing: %s", dest, strerror(errno));
        not_reached();
    }
    ...
    if (outfd < 0) {
        errp(226, __func__, "failed to obtain file descriptor for %s: %s", dest, strerror(errno));
        not_reached();
    }
    ...
    if (outbytes != inbytes) {
        errp(227, __func__, "error: wrote %ju bytes out of expected %ju bytes",
    ...
    ret = fclose(out_file);
    if (ret < 0) {
	errp(228, __func__, "fclose error for %s: %s", dest, strerror(errno));
	not_reached();
    }
    ...
    if (out_file == NULL) {
        ...
        err(229, __func__, "couldn't open dest file for reading: %s: %s", dest, strerror(errno));
        not_reached();
    }
    ...
    if (copy == NULL) {
        err(230, __func__, "couldn't read in dest file: %s", dest);
        not_reached();
    }
    ...
    ret = fclose(out_file);
    if (ret < 0) {
	errp(231, __func__, "fclose error for %s: %s", dest, strerror(errno));
	not_reached();
    }
     ...
     */
    if (outbytes != inbytes) {
        err(232, __func__, "error: read %ju bytes out of expected %ju bytes",
    ...
    if (memcmp(copy, buf, inbytes) != 0) {
        err(233, __func__, "copy of src file %s is not the same as the contents of the dest file %s", src, dest);
        not_reached();
    } else {
    ...
    if (copy_mode) {
        ...
        if (ret != 0) {
            errp(234, __func__, "fchmod(2) failed to set source file %s mode %o on %s: %s", src, in_st.st_mode,
                    dest, strerror(errno));
            not_reached();
        }
        ...
        if (ret != 0) {
            errp(235, __func__, "failed to get stat info for %s, stat returned: %s", dest, strerror(errno));
            not_reached();
        }
        ...
        if (in_st.st_mode != out_st.st_mode) {
            err(236, __func__, "failed to copy st_mode %o from %s to %s: %o != %o", in_st.st_mode, src, dest, in_st.st_mode,
                    out_st.st_mode);
            not_reached();
        }
    } else {
        ...
        if (ret != 0) {
            errp(237, __func__, "fchmod(2) failed to set requested mode on %s: %s", dest, strerror(errno));
            not_reached();
        }
    ...
    ret = close(outfd);
    if (ret < 0) {
        errp(238, __func__, "close(outfd) failed: %s", strerror(errno));
        not_reached();
    }
...
mkdirs(int dirfd, const char *str, mode_t mode)
{
     ...
     */
    if (str == NULL) {
        err(239, __func__, "str (path) is NULL");
        not_reached();
    }

    len = strlen(str);
    if (len <= 0) {
        err(240, __func__, "str (path) is empty");
        not_reached();
    }
    ...
    errno = 0; /* pre-clear errno for errp() */
    if (fchdir(dirfd) != 0) {
        errp(243, __func__, "failed to change to parent directory");
        not_reached();
    }
    ...
    if (p == NULL) {
        ...
        if (mkdir(dup, 0) != 0) {
            if (errno != EEXIST) {
                errp(244, __func__, "mkdir() of %s failed with: %s", dup, strerror(errno));
                not_reached();
            } else {
                /*
                 * now set modes
                ...
                errno = 0; /* pre-clear errno for errp */
                if (chmod(dup, mode) != 0) {
                    errp(245, __func__, "chmod(\"%s\", %o) failed", dup, mode);
                    not_reached();
                } else {
            ...
            errno = 0; /* pre-clear errno for errp */
            if (chmod(dup, mode) != 0) {
                errp(246, __func__, "chmod(\"%s\", %o) failed", dup, mode);
                not_reached();
            } else {
                dbg(DBG_HIGH, "set modes %o on %s", mode, dup);
            }
        ...
        if (mkdir(p, 0) != 0) {
            if (errno != EEXIST) {
                errp(247, __func__, "mkdir() of %s failed with: %s", p, strerror(errno));
                not_reached();
            } else {
                /*
                 * now set modes
            ...
            errno = 0; /* pre-clear errno for errp */
            if (chmod(dup, mode) != 0) {
                errp(249, __func__, "chmod(\"%s\", %o) failed", dup, mode);
                not_reached();
            } else {
                dbg(DBG_HIGH, "set mode %o on %s", mode, dup);
            }
        ...
        errno = 0; /* pre-clear errno for errp() */
        if (chdir(p) != 0) {
            errp(10, __func__, "failed to change to %s", p);
            not_reached();
        }
        ...
        for (p = strtok_r(NULL, "/", &saveptr); p != NULL; p = strtok_r(NULL, "/", &saveptr)) {
            ...
            if (mkdir(p, 0) != 0) {
                if (errno != EEXIST) {
                    errp(11, __func__, "mkdir() of %s failed with: %s", p, strerror(errno));
                    not_reached();
                } else {
                    /*
                     * now set modes
                    ...
                    errno = 0; /* pre-clear errno for errp */
                    if (chmod(p, mode) != 0) {
                        errp(12, __func__, "chmod(\"%s\", %o) failed", p, mode);
                        not_reached();
                    } else {
                ...
                errno = 0; /* pre-clear errno for errp */
                if (chmod(p, mode) != 0) {
                    errp(13, __func__, "chmod(\"%s\", %o) failed", p, mode);
                    not_reached();
                } else {
                    dbg(DBG_HIGH, "set mode %o on %s", mode, p);
                }
            ...
            dir = chdir(p);
            if (dir < 0) {
                errp(14, __func__, "failed to open directory %s", p);
                not_reached();
            }
        }
    }
    ...
    errno = 0; /* pre-clear errno for errp() */
    if (fchdir(cwd) != 0) {
        errp(15, __func__, "failed to change back to previous directory");
        not_reached();
    }
    ...
    errno = 0; /* pre-clear errno for errp() */
    if (close(cwd) != 0) {
        errp(16, __func__, "failed to close(cwd): %s", strerror(errno));
        not_reached();
    }
...
path_has_component(char const *path, char const *name)
{
    ...
    if (path_dup == NULL) {
        errp(20, __func__, "duplicating %s failed", path);
        not_reached();
    }
...
calloc_path(char const *dirname, char const *filename)
{
     ...
     */
    if (filename == NULL) {
	err(22, __func__, "filename is NULL");
	not_reached();
    }
	...
	buf = strdup(filename);
	if (buf == NULL) {
	    errp(23, __func__, "strdup of filename failed: %s", filename);
	    not_reached();
	}
	...
	errno = 0;		/* pre-clear errno for errp() */
	if (buf == NULL) {
	    errp(24, __func__, "calloc of %ju bytes failed", (uintmax_t)len);
	    not_reached();
	}
	...
	if (ret < 0) {
	    errp(25, __func__, "snprintf returned: %zu < 0", len);
	    not_reached();
	}
    }
     ...
     */
    if (buf == NULL) {
	errp(26, __func__, "function attempted to return NULL");
	not_reached();
    }
    return buf;
}
...
open_dir_file(char const *dir, char const *file)
{
     ...
     */
    if (file == NULL) {
	err(27, __func__, "called with NULL file");
	not_reached();
    }
	 ...
	 */
	if (!exists(dir)) {
	    err(29, __func__, "directory does not exist: %s", dir);
	    not_reached();
	}
	if (!is_dir(dir)) {
	    err(30, __func__, "is not a directory: %s", dir);
	    not_reached();
	}
	if (!is_exec(dir)) {
	    err(31, __func__, "directory is not searchable: %s", dir);
	    not_reached();
	}
	...
	ret = chdir(dir);
	if (ret < 0) {
	    errp(32, __func__, "cannot cd %s", dir);
	    not_reached();
	}
    }
     ...
     */
    if (!exists(file)) {
	err(33, __func__, "file does not exist: %s", file);
	not_reached();
    }
    if (!is_file(file)) {
	err(34, __func__, "file is not a regular file: %s", file);
	not_reached();
    }
    if (!is_read(file)) {
	err(35, __func__, "file is not a readable file: %s", file);
	not_reached();
    }
    ...
    if (ret_stream == NULL) {
	errp(36, __func__, "cannot open file: %s", file);
	not_reached();
    }
	...
	ret = fchdir(cwd);
	if (ret < 0) {
	    errp(37, __func__, "cannot fchdir to the previous current directory");
	    not_reached();
	}
	errno = 0;                  /* pre-clear errno for errp() */
	ret = close(cwd);
	if (ret < 0) {
	    errp(38, __func__, "close of previous current directory failed");
	    not_reached();
	}
    }
...
count_char(char const *str, int ch)
{
     ...
     */
    if (str == NULL) {
	err(39, __func__, "given NULL str");
	not_reached();
    }
...
#define UTIL_TEST_VERSION "1.0.16 2025-02-18" /* version format: major.minor YYYY-MM-DD */
...
main(int argc, char **argv)
{
	...
	case 'h':	/* -h - write help, to stderr and exit 0 */
	    fprintf_usage(40, stderr, usage, program, UTIL_TEST_VERSION, JPARSE_UTILS_VERSION, JPARSE_UTF8_VERSION,
	...
	case ':':
	    ...
	    fprintf_usage(41, stderr, usage, program, UTIL_TEST_VERSION, JPARSE_UTILS_VERSION, JPARSE_UTF8_VERSION,
                    JPARSE_LIBRARY_VERSION); /*ooo*/
	    not_reached();
	    break;
	case '?':
	...
	default:
	    ...
	    fprintf_usage(43, stderr, usage, program, UTIL_TEST_VERSION, JPARSE_UTILS_VERSION,
    ...
    if (buf == NULL) {
	errp(44, __func__, "calloc_path(\"%s\", \"%s\") returned NULL", dirname, filename);
	not_reached();
    } else if (strcmp(buf, "foo/bar") != 0) {
	err(45, __func__, "buf: %s != %s/%s", buf, dirname, filename);
	not_reached();
    } else {
    ...
    buf = calloc_path(dirname, filename);
    if (buf == NULL) {
	errp(46, __func__, "calloc_path(NULL, \"%s\") returned NULL", filename);
	not_reached();
    } else if (strcmp(buf, "bar") != 0) {
	err(47, __func__, "buf: %s != %s", buf, filename);
	not_reached();
    } else {
    ...
    if (sanity != PATH_OK) {
        err(48, __func__, "sane_relative_path(\"%s\", 99, 25, 4, fale): expected PATH_OK, got: %s",
    ...
    sanity = sane_relative_path(relpath, 99, 25, 4, false);
    if (sanity != PATH_OK) {
        err(49, __func__, "sane_relative_path(\"%s\", 99, 25, 4, false): expected PATH_OK, got: %s",
    ...
    if (sanity != PATH_ERR_PATH_EMPTY) {
        err(50, __func__, "sane_relative_path(\"%s\", 99, 25, 2, false): expected PATH_ERR_PATH_EMPTY, got: %s",
                relpath, path_sanity_name(sanity));
        not_reached();
    } else {
    ...
    if (sanity != PATH_ERR_PATH_TOO_LONG) {
        err(51, __func__, "sane_relative_path(\"%s\", 2, 99, 2, false): expected PATH_ERR_PATH_TOO_LONG, got: %s",
                relpath, path_sanity_name(sanity));
        not_reached();
    } else {
    ...
    if (sanity != PATH_ERR_MAX_PATH_LEN_0) {
        err(52, __func__, "sane_relative_path(\"%s\", 0, 25, 2, false): expected PATH_ERR_MAX_PATH_LEN_0, got: %s",
                relpath, path_sanity_name(sanity));
        not_reached();
    } else {
    ...
    if (sanity != PATH_ERR_MAX_DEPTH_0) {
        err(53, __func__, "sane_relative_path(\"%s\", 99, 25, 0, false): expected PATH_ERR_MAX_DEPTH_0, got: %s",
                relpath, path_sanity_name(sanity));
        not_reached();
    } else {
    ...
    if (sanity != PATH_ERR_NOT_RELATIVE) {
        err(54, __func__, "sane_relative_path(\"%s\", 99, 25, 4, false): expected PATH_ERR_NOT_RELATIVE, got: %s",
                relpath, path_sanity_name(sanity));
        not_reached();
    } else {
    ...
    if (sanity != PATH_ERR_NAME_TOO_LONG) {
        err(55, __func__, "sane_relative_path(\"%s\", 99, 25, 4, false): expected PATH_ERR_NAME_TOO_LONG, got: %s",
                relpath, path_sanity_name(sanity));
        not_reached();
    } else {
    ...
    if (sanity != PATH_ERR_MAX_NAME_LEN_0) {
        err(56, __func__, "sane_relative_path(\"%s\", 99, 0, 2, false): expected PATH_ERR_MAX_NAME_LEN_0, got: %s",
                relpath, path_sanity_name(sanity));
        not_reached();
    } else {
    ...
    if (sanity != PATH_ERR_PATH_TOO_DEEP) {
        err(57, __func__, "sane_relative_path(\"%s\", 99, 25, 1, false): expected PATH_ERR_PATH_TOO_DEEP, got: %s",
                relpath, path_sanity_name(sanity));
        not_reached();
    } else {
    ...
    if (sanity != PATH_ERR_NOT_POSIX_SAFE) {
        err(58, __func__, "sane_relative_path(\"%s\", 99, 25, 4, false): expected PATH_ERR_NOT_POSIX_SAFE, got: %s",
                relpath, path_sanity_name(sanity));
        not_reached();
    } else {
    ...
    sanity = sane_relative_path(relpath, 99, 25, 4, false);
    if (sanity != PATH_ERR_NOT_POSIX_SAFE) {
        err(59, __func__, "sane_relative_path(\"%s\", 99, 25, 4, false): expected PATH_ERR_NOT_POSIX_SAFE, got: %s",
                relpath, path_sanity_name(sanity));
        not_reached();
    } else {
        fdbg(stderr, DBG_MED, "sane_relative_path(\"%s\", 99, 25, 4, false) == PATH_ERR_NOT_POSIX_SAFE", relpath);
    ...
    sanity = sane_relative_path(relpath, 99, 25, 4, false);
    if (sanity != PATH_OK) {
        err(61, __func__, "sane_relative_path(\"%s\", 99, 25, 4, false): expected PATH_OK, got: %s",
                relpath, path_sanity_name(sanity));
        not_reached();
    } else {
        fdbg(stderr, DBG_MED, "sane_relative_path(\"%s\", 99, 25, 4, false) == PATH_OK", relpath);
    ...
    if (sanity != PATH_ERR_PATH_TOO_DEEP) {
        err(62, __func__, "sane_relative_path(\"%s\", 99, 25, 3, false): expected PATH_ERR_PATH_TOO_DEEP, got: %s",
                relpath, path_sanity_name(sanity));
        not_reached();
    } else {
    ...
    if (sanity != PATH_OK) {
        err(63, __func__, "sane_relative_path(\"%s\", 99, 25, 3, true): expected PATH_OK, got: %s",
                relpath, path_sanity_name(sanity));
        not_reached();
    } else {
    ...
    sanity = sane_relative_path(relpath, 99, 25, 3, false);
    if (sanity != PATH_ERR_NOT_POSIX_SAFE) {
        err(64, __func__, "sane_relative_path(\"%s\", 99, 25, 3, false): expected PATH_ERR_NOT_POSIX_SAFE, got: %s",
                relpath, path_sanity_name(sanity));
        not_reached();
    } else {
    ...
    sanity = sane_relative_path(relpath, 99, 25, 3, true);
    if (sanity != PATH_ERR_NOT_RELATIVE) {
        err(65, __func__, "sane_relative_path(\"%s\", 99, 25, 3, true): expected PATH_ERR_NOT_RELATIVE, got: %s",
                relpath, path_sanity_name(sanity));
        not_reached();
    } else {
    ...
    comps = count_dirs(relpath);
    if (comps != 1) {
        err(85, __func__, "count_dirs(\"%s\"): %ju != 1", relpath, comps);
        not_reached();
    } else {
    ...
    comps = count_dirs(relpath);
    if (comps != 0) {
        err(89, __func__, "count_dirs(\"%s\"): %ju != 0", relpath, comps);
        not_reached();
    } else {
    ...
    comps = count_dirs(relpath);
    if (comps != 2) {
        err(90, __func__, "count_dirs(\"%s\"): %ju != 2", relpath, comps);
        not_reached();
    } else {
        fdbg(stderr, DBG_MED, "count_dirs(\"%s\") == 2", relpath);
    }
    ...
    comps = count_dirs(relpath);
    if (comps != 4) {
        err(91, __func__, "count_dirs(\"%s\"): %ju != 4", relpath, comps);
        not_reached();
    } else {
    ...
    comps = count_dirs(relpath);
    if (comps != 4) {
        err(93, __func__, "count_dirs(\"%s\"): %ju != 4", relpath, comps);
        not_reached();
    } else {
        fdbg(stderr, DBG_MED, "count_dirs(\"%s\") == 4", relpath);
    }
    ...
    comps = count_dirs(relpath);
    if (comps != 3) {
        err(95, __func__, "count_dirs(\"%s\"): %ju != 3", relpath, comps);
        not_reached();
    } else {
    ...
    comps = count_dirs(relpath);
    if (comps != 3) {
        err(97, __func__, "count_dirs(\"%s\"): %ju != 3", relpath, comps);
        not_reached();
    } else {
        fdbg(stderr, DBG_MED, "count_dirs(\"%s\") == 3", relpath);
    }
    ...
    if (comps != 2) {
        err(98, __func__, "count_comps(\"%s\", ',', true): %ju != 2", relpath, comps);
        not_reached();
    } else {
    ...
    if (!path_has_component(relpath, "baz")) {
        err(99, __func__, "path_has_component(\"%s\", \"baz\") returned false: expected true", relpath);
        not_reached();
    } else {
    ...
    if (!path_has_component(relpath, "bar")) {
        err(100, __func__, "path_has_component(\"%s\", \"bar\") returned false: expected true", relpath);
        not_reached();
    } else {
    ...
    relpath = "foo//bar/baz";
    if (!path_has_component(relpath, "bar")) {
        err(101, __func__, "path_has_component(\"%s\", \"bar\") returned false: expected true", relpath);
        not_reached();
    } else {
        fdbg(stderr, DBG_MED, "path_has_component(\"%s\", \"bar\") == true", relpath);
    }
    ...
    relpath = "foo//bar/baz";
    if (!path_has_component(relpath, "baz")) {
        err(102, __func__, "path_has_component(\"%s\", \"baz\") returned false: expected true", relpath);
        not_reached();
    } else {
        fdbg(stderr, DBG_MED, "path_has_component(\"%s\", \"baz\") == true", relpath);
    }
    ...
    if (!path_has_component(relpath, ".git")) {
        err(103, __func__, "path_has_component(\"%s\", \".git\") returned false: expected true", relpath);
        not_reached();
    } else {
    ...
    if (relpath != NULL) {
        ...
        if (stat(relpath, &in_st) != 0) {
            errp(104, __func__, "couldn't stat file %s", relpath);
            not_reached();
        }
        ...
        if (stat("util_test.copy.c", &out_st) != 0) {
            errp(105, __func__, "couldn't stat file util_test.copy.c");
            not_reached();
        }

        if (!is_mode("util_test.copy.c", in_st.st_mode)) {
            err(106, __func__, "copyfile() failed to copy st_mode of file %s", relpath);
            not_reached();
        } else {
        ...
        if (unlink("util_test.copy.c") != 0) {
            errp(107, __func__, "unable to delete file util_test.copy.c");
            not_reached();
        } else {
    ...
    if (relpath != NULL) {
        ...
        errno = 0;  /* pre-clear errno for errp() */
        if (stat("util_test.copy.c", &out_st) != 0) {
            errp(108, __func__, "failed to stat util_test.copy.c");
            not_reached();
        }

        if ((out_st.st_mode & (S_IRWXU | S_IRWXG | S_IRWXO)) != (S_IRUSR|S_IWUSR)) {
            err(109, __func__, "copyfile() failed to set S_IRUSR|S_IWUSR on util_test.copy.c");
            not_reached();
        } else {
        ...
        errno = 0;      /* pre-clear errno for errp() */
        if (unlink("util_test.copy.c") != 0) {
            errp(110, __func__, "unable to delete file util_test.copy.c");
            not_reached();
        } else {
            fdbg(stderr, DBG_MED, "successfully deleted copied file util_test.copy.c");
        }
    ...
    if (stat("util.o", &in_st) != 0) {
        errp(111, __func__, "failed to stat util.o");
        not_reached();
    }
    /*
    ...
    if (stat("util.copy.o", &out_st) != 0) {
        errp(112, __func__, "failed to stat util.copy.o");
        not_reached();
    }

    if (!is_mode("util.copy.o", in_st.st_mode)) {
        err(113, __func__, "util.o st_mode != util.copy.o st_mode: %o != %o", in_st.st_mode, out_st.st_mode);
        not_reached();
    } else {
    ...
    if (unlink("util.copy.o") != 0) {
        errp(114, __func__, "unable to delete file util.copy.o");
        not_reached();
    } else {
        fdbg(stderr, DBG_MED, "successfully deleted copied file util_test.copy.c");
    }
    ...
    if (!exists(relpath)) {
        err(115, __func__, "%s does not exist", relpath);
        not_reached();
    } else if (!is_dir(relpath)) {
        err(116, __func__, "%s is not a directory", relpath);
        not_reached();
    } else {
    ...
    if (!dir_exists) {
        if (!is_mode(relpath, 0755)) {
            err(117, __func__, "failed to set mode of %s to %o", relpath, 0755);
            not_reached();
        } else {
    ...
    mkdirs(-1, relpath, 0755);
    if (!exists(relpath)) {
        err(118, __func__, "%s does not exist", relpath);
        not_reached();
    } else if (!is_dir(relpath)) {
        err(119, __func__, "%s is not a directory", relpath);
        not_reached();
    } else {
        fdbg(stderr, DBG_MED, "%s is a directory", relpath);
    }
    ...
    if (!dir_exists) {
        if (!is_mode(relpath, 0755)) {
            err(120, __func__, "failed to set mode of %s to %o", relpath, 0755);
            not_reached();
        } else {
            fdbg(stderr, DBG_MED, "%s is mode 07555", relpath);
        }
        ...
        if (!has_mode(relpath, 0755)){
            err(121, __func__, "%s does not have bits %0o set", relpath, 0755);
            not_reached();
        } else {
        ...
        if (!has_mode(relpath, S_IRWXU)) {
            err(122, __func__, "%s does not have bits %0o set", relpath, S_IRWXU);
            not_reached();
        } else {
        ...
        if (has_mode(relpath, S_ISUID)) {
            err(123, __func__, "%s has set user id on execution", relpath);
            not_reached();
        } else {
            ...
            switch (ent->fts_info) {
                case FTS_F:
                    ...
                    if (is_file(p)) {
                        ...
                        err(126, __func__, "%s reported as FTS_FILE !is_file(\"%s\")", p, p);
                        not_reached();
                    }
                    break;
                case FTS_D:
                    ...
                    if (is_dir(p)) {
                        ...
                        err(128, __func__, "%s reported as FTS_D !is_dir(\"%s\")", p, p);
                        not_reached();
                    }
                    break;
                case FTS_SL:
                    ...
                    if (is_symlink(p)) {
                        ...
                        err(129, __func__, "%s reported as FTS_SL !is_symlink(\"%s\")", p, p);
                        not_reached();
                    }
                    break;
                case FTS_SLNONE:
                    ...
                    break;
                case FTS_NS:
                    err(130, __func__, "no stat(2) info available for path %s in tree", p);
                    not_reached();
                    break;
                case FTS_NSOK:
                    err(131, __func__, "encountered FTS_NSOK for path %s in tree: FTS_NOSTAT set!", p);
                    not_reached();
                    break;
            }
    ...
    if (ent == NULL) {
        err(132, __func__, "read_fts() returned a NULL pointer on \"test_jparse\"");
        not_reached();
    } else {
        do {
            char *p = fts_path(ent);
            if (ent == NULL) {
                err(133, __func__, "fts_path(ent) returned NULL");
                not_reached();
            }
            switch (ent->fts_info) {
                case FTS_F:
                ...
                case FTS_SLNONE:
                    ...
                    break;
                case FTS_NS:
                    err(134, __func__, "no stat(2) info available");
                    not_reached();
                    break;
                case FTS_NSOK:
                    err(135, __func__, "encountered FTS_NSOK for path %s in tree: FTS_NOSTAT set!", p);
                    not_reached();
                    break;
            }
    ...
    if (paths_found == NULL) {
        err(136, __func__, "didn't find any paths in the paths array");
        not_reached();
    }
    ...
    if (paths_found == NULL) {
        err(138, __func__, "didn't find any paths in the paths array");
        not_reached();
    }
    ...
    if (len > 0) {
        for (j = 0; j < len; ++j) {
            ...
            name = dyn_array_value(paths_found, char *, j);
            if (name == NULL) {	/* paranoia */
                err(139, __func__, "found NULL pointer at paths_found[%ju]", (uintmax_t)j);
                not_reached();
            }
    ...
    if (paths_found == NULL) {
        err(140, __func__, "didn't find any files in the paths array");
        not_reached();
    }
    ...
    if (len > 0) {
        for (j = 0; j < len; ++j) {
            ...
            name = dyn_array_value(paths_found, char *, j);
            if (name == NULL) {	/* paranoia */
                err(141, __func__, "found NULL pointer at paths_found[%ju]", (uintmax_t)j);
                not_reached();
            }
    ...
    if (paths_found == NULL) {
        err(142, __func__, "didn't find any files in the paths array");
        not_reached();
    }
    ...
    if (len > 0) {
        for (j = 0; j < len; ++j) {
            ...
            name = dyn_array_value(paths_found, char *, j);
            if (name == NULL) {	/* paranoia */
                err(143, __func__, "found NULL pointer at paths_found[%ju]", (uintmax_t)j);
                not_reached();
            }
    ...
    if (paths_found == NULL) {
        err(144, __func__, "didn't find any files in the paths array");
        not_reached();
    }
    ...
    if (len > 0) {
        for (j = 0; j < len; ++j) {
            ...
            name = dyn_array_value(paths_found, char *, j);
            if (name == NULL) {	/* paranoia */
                err(145, __func__, "found NULL pointer at paths_found[%ju]", (uintmax_t)j);
                not_reached();
            }
    ...
    if (paths_found == NULL) {
        err(146, __func__, "didn't find any paths in the paths array");
        not_reached();
    }
    ...
    if (len > 0) {
        ...
        for (j = 0; j < len; ++j) {
            ...
            name = dyn_array_value(paths_found, char *, j);
            if (name == NULL) {	/* paranoia */
                err(147, __func__, "found NULL pointer at paths_found[%ju]", (uintmax_t)j);
                not_reached();
            }
    ...
    if (paths_found != NULL) {
        err(148, __func__, "found unexpected socket under test_jparse/");
        not_reached();
    }
    ...
    if (paths_found != NULL) {
        err(149, __func__, "found unexpected character device under test_jparse/");
        not_reached();
    }
    ...
    if (paths_found != NULL) {
        err(150, __func__, "found unexpected block device under test_jparse/");
        not_reached();
    }
    ...
    if (paths_found != NULL) {
        err(151, __func__, "found unexpected FIFO under test_jparse/");
        not_reached();
    }
    ...
    if (paths_found != NULL) {
        ...
        for (j = 0; j < len; ++j) {
            name = dyn_array_value(paths_found, char *, j);
            if (name == NULL) {
                err(152, __func__, "found NULL pointer in paths_found (sockets) array");
                not_reached();
            }
         ...
         * make it an error
         */
        err(153, __func__, "found unexpected file type under test_jparse/");
        not_reached();
    }
